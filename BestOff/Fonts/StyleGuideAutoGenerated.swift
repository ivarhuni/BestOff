//
//  UIFont+Additions.swift
//  Reykjavík Grapevine
//
//  Generated on Zeplin. (16/04/2019).
//  Copyright (c) 2019 __MyCompanyName__. All rights reserved.
//

import UIKit


//MARK: Generated Font Code from Zeplin
extension UIFont {

  class var hugeHeadline: UIFont {
    return UIFont(name: "Poppins-Bold", size: 40.0)!
  }

  class var categoryType: UIFont {
    return UIFont(name: "Poppins-Bold", size: 24.0)!
  }

  class var catDtlItemTitle: UIFont {
    return UIFont(name: "Poppins-Bold", size: 22.0)!
  }

  class var guideHeadline: UIFont {
    return UIFont(name: "Poppins-Bold", size: 22.0)!
  }
    
  class var guideHeadlineFav: UIFont {
    return UIFont(name: "Poppins-Bold", size: 21.0)!
  }

  class var itemTitle: UIFont {
    return UIFont(name: "Poppins-Bold", size: 22.0)!
  }
    
    class var backLblTitle: UIFont {
        return UIFont(name: "Poppins-Bold", size: 20.0)!
    }

  class var catDtlItemSeperator: UIFont {
    return UIFont(name: "Poppins-Bold", size: 20.0)!
  }

  class var favouriteOn: UIFont {
    return UIFont(name: "Poppins-Bold", size: 20.0)!
  }

  class var catItemAbout: UIFont {
    return UIFont(name: "Poppins-Medium", size: 20.0)!
  }

  class var wordsBy: UIFont {
    return UIFont(name: "Poppins-Bold", size: 18.0)!
  }

  class var categoryHeadline: UIFont {
    return UIFont(name: "Poppins-Medium", size: 19.0)!
  }

  class var favCatTitle: UIFont {
    return UIFont(name: "Poppins-Medium", size: 18.0)!
  }

  class var favEditTxt: UIFont {
    return UIFont(name: "Poppins-Medium", size: 18.0)!
  }
    
    class var boldImgTitle: UIFont {
        return UIFont(name: "Poppins-Bold", size: 18.0)!
    }
    
    class var cellItemName: UIFont {
        return UIFont(name: "Poppins-Bold", size: 20.0)!
    }
    
  class var cellItemText: UIFont {
    return UIFont(name: "Poppins-Medium", size: 14.0)!
  }

  class var favouriteOff: UIFont {
    return UIFont(name: "Poppins-Medium", size: 18.0)!
  }

  class var categoryImageTitle: UIFont {
    return UIFont(name: "Poppins-Bold", size: 16.0)!
  }

  class var redDirectionText: UIFont {
    return UIFont(name: "Poppins-Bold", size: 16.0)!
  }

  class var catItemType: UIFont {
    return UIFont(name: "Poppins-Medium", size: 16.0)!
  }

  class var favBtnSaveChanges: UIFont {
    return UIFont(name: "Poppins-Medium", size: 16.0)!
  }

  class var catDtlItemYears: UIFont {
    return UIFont(name: "Poppins-Bold", size: 14.0)!
  }

  class var gvImageHeadline: UIFont {
    return UIFont(name: "Poppins-Bold", size: 15.0)!
  }

  class var authorName: UIFont {
    return UIFont(name: "Poppins-Medium", size: 14.0)!
  }
    
  class var smallAuthorName: UIFont{
    return UIFont(name: "Poppins-Medium", size: 12.0)!
  }

  class var browseMoreCatItems: UIFont {
    return UIFont(name: "Poppins-Medium", size: 16.0)!
  }

  class var catDtlItemAddressTitle: UIFont {
    return UIFont(name: "Poppins-Medium", size: 14.0)!
  }

  class var catDtlItemBottomBack: UIFont {
    return UIFont(name: "Poppins-Medium", size: 14.0)!
  }

  class var catDtlItemYearWinner: UIFont {
    return UIFont(name: "Poppins-Medium", size: 14.0)!
  }

  class var catItemImgName: UIFont {
    return UIFont(name: "Poppins-Medium", size: 14.0)!
  }
    
    class var catItemImgNameBigger: UIFont {
        return UIFont(name: "Poppins-Bold", size: 16.0)!
    }

  class var gvImageText: UIFont {
    return UIFont(name: "Poppins-Medium", size: 17.0)!
  }

  class var guideItem: UIFont {
    return UIFont(name: "Poppins-Medium", size: 14.0)!
  }

  class var load: UIFont {
    return UIFont(name: "Poppins-Medium", size: 14.0)!
  }

  class var rBlueBtnBorder: UIFont {
    return UIFont(name: "Poppins-Medium", size: 14.0)!
  }

  class var rPeachBtnText: UIFont {
    return UIFont(name: "Poppins-Medium", size: 14.0)!
  }

  class var rRedBtnText: UIFont {
    return UIFont(name: "Poppins-Medium", size: 15.0)!
  }

  class var rvGrSmallBlack: UIFont {
    return UIFont(name: "Poppins-Medium", size: 14.0)!
  }

  class var rYlwBtnText: UIFont {
    return UIFont(name: "Poppins-Medium", size: 14.0)!
  }

  class var swipeRightTxt: UIFont {
    return UIFont(name: "Poppins-Medium", size: 14.0)!
  }

  class var smallGrayRvg: UIFont {
    return UIFont(name: "Poppins-Medium", size: 14.0)!
  }

  class var favBtnRemove: UIFont {
    return UIFont(name: "Poppins-Bold", size: 12.0)!
  }

}

extension UIColor {
    
    @nonobjc class var red: UIColor {
        return UIColor(red: 209.0 / 255.0, green: 65.0 / 255.0, blue: 68.0 / 255.0, alpha: 1.0)
    }
    
    @nonobjc class var blackText: UIColor {
        return UIColor(white: 50.0 / 255.0, alpha: 1.0)
    }
    
    @nonobjc class var greyDot: UIColor {
        return UIColor(white: 175.0 / 255.0, alpha: 1.0)
    }
    
    @nonobjc class var greySep: UIColor {
        return UIColor(white: 200.0 / 255.0, alpha: 1.0)
    }
    
    @nonobjc class var greyText: UIColor {
        return UIColor(white: 150.0 / 255.0, alpha: 1.0)
    }
    
    @nonobjc class var greyBrowse: UIColor {
        return UIColor(white: 100.0 / 255.0, alpha: 1.0)
    }
    
    @nonobjc class var peachBtnBorder: UIColor {
        return UIColor(red: 221.0 / 255.0, green: 129.0 / 255.0, blue: 102.0 / 255.0, alpha: 1.0)
    }
    
    @nonobjc class var yellowBtnBorder: UIColor {
        return UIColor(red: 212.0 / 255.0, green: 172.0 / 255.0, blue: 71.0 / 255.0, alpha: 1.0)
    }
    
    @nonobjc class var blueBtnBorder: UIColor {
        return UIColor(red: 56.0 / 255.0, green: 167.0 / 255.0, blue: 181.0 / 255.0, alpha: 1.0)
    }
    
    @nonobjc class var white: UIColor {
        return UIColor(white: 1.0, alpha: 1.0)
    }
    
    @nonobjc class var black: UIColor {
        return UIColor(white: 0.0, alpha: 1.0)
    }
    
}
